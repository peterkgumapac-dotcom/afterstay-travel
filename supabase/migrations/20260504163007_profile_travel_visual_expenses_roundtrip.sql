-- Profile spend and distance must be computed from live trip data:
-- expenses for money, and round-trip route estimates when exact paired legs
-- are not available. lifetime_stats remains only a no-cache fallback.
    ...
    coalesce(round(sum(
      case
        when lat is null or lng is null then 0
          ...
          * case
            -- Until the cache stores paired flight legs, a completed trip's
            -- profile distance should represent the travel loop, not one-way.
            when exists (
              select 1
